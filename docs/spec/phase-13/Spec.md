...
需要的是**空间维度**的隔离:同一仓库同时挂多个工作目录、共享版本库、各自一个分支。这就是 Git Worktree (Git 2.5+) 的能力。本章在 cortex 中封装一层 Worktree 管理逻辑,把这块拼图补给 SubAgent,让后台 / 并行场景安全可用。

cortex 现有相关基础设施:
- ch13 SubAgent 已支持 frontmatter (`dev.cortex.subagent.Parser`),解析 `name/description/tools/disallowedTools/model/maxTurns/permissionMode/background` 等字段
- ch13 `agent.AgentTool#execute` 已是子 Agent 启动入口,本章在此处插桩 isolation 分支
- ch08 文件读缓存以绝对路径作为 key
- ch10 `dev.cortex.command` 已有 slash 命令注册系统
- `.gitignore` 已忽略 `.cortex/sessions/` 等子目录,本章扩展把 `.cortex/worktrees/` 也忽略
...
- **G3**: Worktree 目录统一落在仓库内不被追踪的位置 `.cortex/worktrees/<flatSlug>/`,分支名前缀 `worktree-<flatSlug>`,嵌套 slug 的 `/` 替换为 `+` 避免 Git D/F 冲突
- **G4**: 创建后做四类环境初始化——A 复制本地配置 (`.cortex/config.yaml` / `.cortex/settings.local.yaml`)、B 配置子目录的 git hooks (`core.hooksPath` 不自动继承)、C 软链 `node_modules` / `.venv` / `vendor` 等大目录、D 按项目根 `.worktreeinclude` 复制被忽略但运行需要的文件;均为 best-effort,失败只警告不中断创建
...
- **G11**: `WorktreeSession` 持久化到 `.cortex/worktree_session.json`,cortex 启动时读取并校验目录仍存在;退出时写空 JSON `null` 而不是删文件,确保下次启动不误恢复
...
- **F4**: `WorktreeManager` 内部字段——`repoRoot`(绝对路径)、`worktreeDir`(`<repoRoot>/.cortex/worktrees`)、`sessionFile`(`<repoRoot>/.cortex/worktree_session.json`)、`ReentrantLock lock`、`Map<String, Worktree> active`、`WorktreeSession currentSession`
- **F5**: `new WorktreeManager(Path repoRoot)` 构造时——
  - 校验 `repoRoot` 是 git 仓库根目录(`git rev-parse --show-toplevel` 输出与之等);失败抛 `IOException`,cortex 启动允许降级到「Worktree 功能未启用」
...
- **F6**: `manager.create(name, baseRef, manual)`——
  ...
  - 8. 加入 `active`,返回
- **F7**: 创建后设置 A——复制本地配置文件,从 `<repoRoot>/.cortex/config.yaml` 与 `<repoRoot>/.cortex/settings.local.yaml` 复制到 Worktree 同位置(目标已存在跳过,文件不存在跳过)
...
### explicit cwd 工具改造- **F16**: 在 `dev.cortex.tool` 包定义 ctx key 与帮助函数(`ToolContext` 不可变上下文对象封装)——
...
- **F31**: cortex 启动时(`new WorktreeManager` 内),读 `sessionFile` 反序列化;若文件内容为 `null` 或空,`currentSession=null`;若 `worktreePath` 不存在,清空文件并 `currentSession=null`(stderr 警告 "session worktree gone, cleared")
- **F32**: `--resume` (cortex 现有恢复入口)读到已有 session 时,把 `activeCwd` 设置到 `session.worktreePath`,主 Agent 后续工具调用都按 explicit cwd 走
...
- **F34**: cortex 启动时跑一次 `Thread.startVirtualThread(() -> manager.sweepStale(Instant.now().minus(24, HOURS)))`(异步、后台执行),不阻塞启动

### .gitignore 更新- **F35**: 在项目根 `.gitignore` 追加 `.cortex/worktrees/` 与 `.cortex/worktree_session.json` 两行;cortex 启动时若发现 `.gitignore` 不含这两行,**只警告不修改**(尊重用户配置)
...
- **N5**: Worktree session 文件被破坏(非法 JSON)启动时只警告并清空,不阻断 cortex 启动
- **N6**: 与 ch04~ch13 既有测试零破坏——`mvn test` 全绿
- **N7**: 中文友好——错误消息与命令输出全部中文(对齐 cortex 其他模块风格)
...
- Windows 平台特殊支持(symlink 行为在 Windows 上不保证;本期 cortex 以 macOS / Linux 为主)
- 跨 cortex 进程实例的 Worktree 共享(同一仓库同一时刻只支持一个 cortex 实例操作 worktree session)
...
- **AC2**: `manager.create("alice", "HEAD", true)` 在 `.cortex/worktrees/alice/` 下落地 Worktree,分支为 `worktree-alice`
- **AC3**: `manager.create("team/alice", "HEAD", true)` 在 `.cortex/worktrees/team+alice/` 下落地,分支 `worktree-team+alice`
- **AC4**: 已存在 worktree 目录时再调 create 走快速恢复——不调 `git worktree add`,毫秒级返回(单测可断言 git 子进程未启动)
- **AC5**: 创建后设置 A——主仓库存在 `.cortex/settings.local.yaml` 时,Worktree 内同位置出现该文件
...
- **AC20**: `WorktreeSession` 持久化到 `.cortex/worktree_session.json`,启动时读取;指向的 Worktree 目录被外部删除后,启动时清空 session 并 stderr 警告
- **AC21**: 项目编译无错误 (`mvn -q -DskipTests package`)、所有单元测试通过 (`mvn test`)、Spotless 检查通过 (`mvn spotless:check`)
- **AC22**: tmux 实跑——`cortex` 启动 + 触发 `isolation:worktree` 子 Agent 改文件 + 验证主目录 `server.py`(若改的是 `server.py`)未变,Worktree 副本里 `server.py` 已变;Worktree 留盘 / 自动清理符合预期