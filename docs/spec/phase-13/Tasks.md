...
| 操作 | 文件 | 职责 |
|------|------|------|
| 新建 | `src/main/java/com/mewcode/worktree/WorktreeSlug.java` | `validate` + `flatten` |
| 新建 | `src/test/java/dev/mewcode/worktree/WorktreeSlugTest.java` | Slug 校验单测 |
| 新建 | `src/main/java/com/mewcode/worktree/WorktreeSession.java` | record + JSON tag |
| 新建 | `src/main/java/com/mewcode/worktree/SessionStore.java` | JSON 原子读写 |
| 新建 | `src/main/java/com/mewcode/worktree/GitHelper.java` | `gitProcess` + `hasWorktreeChanges` + `resolveHeadShaFromFS` |
| 新建 | `src/test/java/dev/mewcode/worktree/GitHelperTest.java` | git helper 单测 |
| 新建 | `src/main/java/com/mewcode/worktree/WorktreeManager.java` | Manager 类型 + 构造 + list/get/currentSession |
| 新建 | `src/main/java/com/mewcode/worktree/Worktree.java` | record |
| 新建 | `src/main/java/com/mewcode/worktree/ExitAction.java` | enum |
| 新建 | `src/main/java/com/mewcode/worktree/ExitOptions.java` | record |
| 新建 | `src/main/java/com/mewcode/worktree/ExitReport.java` | record |
| 新建 | `src/main/java/com/mewcode/worktree/AutoCleanupReport.java` | record |
| 新建 | `src/main/java/com/mewcode/worktree/WorktreeHasChangesException.java` | 异常 |
| 新建 | `src/main/java/com/mewcode/worktree/PostCreationSetup.java` | A/B/C/D 子步骤 |
| 新建 | `src/main/java/com/mewcode/worktree/WorktreeNaming.java` | randomAgentName + ephemeral 正则 |
| 新建 | `src/test/java/dev/mewcode/worktree/WorktreeCreateTest.java` | create + setup 单测 |
| 新建 | `src/test/java/dev/mewcode/worktree/WorktreeLifecycleTest.java` | 生命周期单测 |
| 新建 | `src/test/java/dev/mewcode/worktree/WorktreeSweepTest.java` | sweepStale 单测 |
| 新建 | `src/test/java/dev/mewcode/worktree/WorktreeManagerTest.java` | NewManager + session 持久化测试 |
| 新建 | `src/test/java/dev/mewcode/tool/Test.java` | resolvePath 单测 |
| 修改 | `src/main/java/com/mewcode/tool/ReadFileTool.java` | 用 `ctx.resolvePath` 解析 path |
| 修改 | `src/main/java/com/mewcode/tool/WriteFileTool.java` | 用 `ctx.resolvePath` 解析 path |
| 修改 | `src/main/java/com/mewcode/tool/EditFileTool.java` | 用 `ctx.resolvePath` 解析 path |
| 修改 | `src/main/java/com/mewcode/tool/GlobTool.java` | 用 `ctx.resolvePath` 解析 root |
| 修改 | `src/main/java/com/mewcode/tool/GrepTool.java` | 用 `ctx.resolvePath` 解析 path |
| 修改 | `src/main/java/com/mewcode/tool/BashTool.java` | `pb.directory(ctx.resolvePath("").toFile())` |
| 修改 | `src/main/java/com/mewcode/subagent/Definition.java` | record 加 `isolation` 字段 |
| 修改 | `src/main/java/com/mewcode/subagent/Parser.java` | 解析 isolation: frontmatter |
| 修改 | `src/test/java/dev/mewcode/subagent/ParserTest.java` | 增加 isolation 单测 |
| 新建 | `src/main/java/com/mewcode/agent/AgentWorktreeRunner.java` | `executeWithWorktree` + `buildWorktreeNotice` |
| 修改 | `src/main/java/com/mewcode/agent/AgentTool.java` | 增加 `wtMgr` 字段 + isolation 分支 |
| 新建 | `src/test/java/dev/mewcode/agent/AgentWorktreeRunnerTest.java` | 单测(stub Manager) |
| 修改 | `src/main/java/com/mewcode/command/Ui.java` | 加 `worktreeAccessor()` 方法 |
| 新建 | `src/main/java/com/mewcode/command/WorktreeAccessor.java` | 接口 + WorktreeSummary |
| 新建 | `src/main/java/com/mewcode/command/WorktreeCommand.java` | `/worktree` handler + 子命令解析 |
| 修改 | `src/main/java/com/mewcode/command/Builtins.java` | 注册 `/worktree` |
| 修改 | `src/test/java/dev/mewcode/command/BuiltinsTest.java` | 加 worktree 注册测试 |
| 新建 | `src/main/java/com/mewcode/tui/TuiWorktreeAccessor.java` | 实现 `WorktreeAccessor` 适配 `WorktreeManager` |
| 修改 | `src/main/java/com/mewcode/tui/MewCodeModel.java` | `worktreeMgr` / `activeCwd` 字段 + 注入 ctx |
| 修改 | `src/main/java/com/mewcode/MewCode.java` | 构造 manager + 注入 AgentTool / TUI + sweepStale |
...
**依赖:** 无
**步骤:**
1. 新建包 `com.mewcode.worktree`,加 package-info 注释
...
2. 实现 `public WorktreeManager(Path repoRoot) throws IOException`:
   ...
   - 初始化 `worktreeDir = repoRoot.resolve(".mewcode/worktrees");`、`sessionFile = repoRoot.resolve(".mewcode/worktree_session.json");`
...
2. 实现 `PostCreationSetup.run(...)`,四个 static 子函数:
   - `copyLocalConfigs(repoRoot, wtPath)`:对 `.mewcode/config.yaml` / `.mewcode/settings.local.yaml`,若主仓存在且 Worktree 不存在,`Files.copy`
...
**依赖:** 无(并行 T1-T7)
**步骤:**
1. 新建/扩展 `com.mewcode.tool.` record,字段 `Optional<Path> cwd`(若已有 ctx record,把 cwd 合并到既有 record 末尾)
...
**依赖:** T6, T8, T10
**步骤:**
1. 新建 `AgentWorktreeRunner`,持 `WorktreeManager wtMgr`(构造器注入);agent 包直接 import `com.mewcode.worktree.*`(worktree 包不依赖 agent,无循环)
...
4. 改 `MewCode.java` 构造 `AgentTool` 时传入 `wtMgr`
...
**文件:** `TuiWorktreeAccessor.java` + `MewCodeModel.java`
**依赖:** T11, T13
**步骤:**
...
2. `MewCodeModel.java`:字段加 `WorktreeManager worktreeMgr`、`Path activeCwd`(null 表示 JVM 当前目录)
3. `MewCodeModel.Builder` 加 `worktreeMgr(WorktreeManager)` 方法;构造时若 `manager.currentSession()` 非 null,设 `activeCwd = Path.of(session.worktreePath())`
4. 实现 `worktreeAccessor()` 方法返回 `TuiWorktreeAccessor` 实例
5. 在主 Agent run 调用入口(找 `MewCodeModel` 里 `agent.run(ctx, conv, mode)` 调用点),前置 `ctx = ctx.withCwd(effectiveCwd());`
...
**文件:** `MewCode.java` + `.gitignore`
**依赖:** T4-T14 全部
**步骤:**
1. `MewCode.java`:在 `var subagentCatalog = SubagentCatalog.load(root);` 后加:
   ```java
...
3. `new MewCodeModel(` 链上新增 `.worktreeMgr(worktreeMgr)`
4. `.gitignore` 追加:
   ```
...
**依赖:** T15
**步骤:**
1. `./gradlew shadowJar` 产出 `build/libs/mewcode.jar`
2. 准备项目级自定义 Agent `.mewcode/agents/worktree-writer.md`(详见 checklist 场景 1)
3. tmux 启动 mewcode(`java -jar build/libs/mewcode.jar`),跑 checklist 端到端场景
4. 通过即标记 T16 完成
