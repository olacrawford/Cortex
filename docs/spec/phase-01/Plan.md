...
- TUI：JLine 3（`org.jline:jline`）+ 自建 `com.cortex.tui.tea` 包（Bubble Tea 风格）——
...
## 架构概览（分层）
1. 入口层 `com.cortex.Cortex` —— 加载配置、启动 TUI Program。
2. 配置层 `com.cortex.config` —— 读取并校验 `.cortex/config.yaml`，给出 providers 列表。
3. LLM 协议层 `com.cortex.llm` —— 定义协议无关的 `LlmClient` 接口与统一消息/流式事件类型；
   `AnthropicClient`、`OpenAiClient` 两个适配器各自封装官方 SDK、统一吐出文本增量（思考增量内部丢弃）。
4. 会话层 `com.cortex.conversation` —— 进程内维护多轮历史，提供完整上下文。
5. 提示词/资源 `com.cortex.prompt` —— 内置 system prompt 与启动 banner（ASCII 猫）。
6. TUI 框架层 `com.cortex.tui.tea` —— 基于 JLine 的 Bubble Tea 风格运行时：
   `Program`（内联渲染 + 事件循环）、`Model`/`Command`/`Message`/`UpdateResult`（Elm 架构）、
   `Style`/`ANSI256Color`（终端样式）。
7. TUI 应用层 `com.cortex.tui` —— `CortexModel`（实现 Model 接口），含状态机（选择/空闲/流式）、
   输入缓冲、对话显示、spinner+计时、provider 选择列表、scrollback 提交机制。

## 数据流（一轮对话）
用户输入 → CortexModel 提交 → `ConversationManager` 追加 user 消息 → 调 `LlmClient.stream(conv, tools)`
→ 得到 `BlockingQueue<StreamEvent>` → virtual thread 消费事件 → 通过 `AgentEvent` 队列投递给 TUI 主循环
→ CortexModel.update() 处理事件、更新 streamBuf → view() 重绘
...
### 模块 config
职责：读取并校验 `.cortex/config.yaml`，产出 providers 列表。
...
对外接口：`PromptBuilder.buildSystemPrompt()`、banner 渲染由 `CortexModel.renderBanner()` 内联。
依赖：无。
...
对外接口：`new CortexModel(providers, mcpServers, hooks)` + `Program(model).run()`。
内部职责：
...
### 入口 `com.cortex.Cortex`
职责：装配与启动。
流程：`ConfigLoader.load(...)` → `new CortexModel(providers)` → `new Program(model).run()`。
...
Cortex.main → ConfigLoader.load(".cortex/config.yaml")
     → 若抛异常：打印可读错误、System.exit(1)
     → new CortexModel(providers, mcpServers, hooks)
     → new Program(model)
...
Program ──poll AgentEvent──> CortexModel.update() → view() → 内联渲染
...
│   ├── Cortex.java               — 入口：加载配置、启动 Program
│   ├── config/
│   │   ├── AppConfig.java         — 配置根类
...
│       │   └── ANSI256Color.java
│       ├── CortexModel.java      — 主 TUI 模型（Model 实现）
│       ├── Styles.java            — 全局样式常量
...
- `.cortex/config.yaml` 含真实密钥，应在 `.gitignore` 忽略；提交一份 `config.yaml.example`。
...
| 构建 | Gradle + Shadow | Kotlin DSL 简洁；Shadow plugin 打 fat jar，`java -jar cortex.jar` 即可运行 |
...
| 配置 | `.cortex/config.yaml` + SnakeYAML；密钥入 `.gitignore` | 用户既定路径；N5 密钥安全 |
