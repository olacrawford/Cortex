...
- [ ] 项目编译无错误：`./gradlew shadowJar`（验证：exit code 0）
- [ ] 所有单元测试通过：`./gradlew -q test`（验证：BUILD SUCCESS，无 FAIL）
- [ ] 格式化检查通过：`./gradlew -q spotless:check`（验证：exit code 0；google-java-format）
...
- [ ] 路径逃逸：项目级 MEWCODE.md 中 `@include ../../outside.md` → 不加载，出现范围警告（验证：`output.contains("路径超出允许范围")`）
