...
| 操作 | 文件 | 职责 |
|------|------|------|
| 新建 | `src/main/java/com/cortex/command/Kind.java` | Kind 枚举 |
| 新建 | `src/main/java/com/cortex/command/Command.java` | Command record + Handler 函数接口 |
| 新建 | `src/main/java/com/cortex/command/CommandRegistry.java` | register/lookup/visible/prefixMatch + 冲突检测 |
| 新建 | `src/test/java/dev/cortex/command/CommandRegistryTest.java` | 注册、冲突抛异常、前缀匹配、visible 排序的单测 |
| 新建 | `src/main/java/com/cortex/command/Dispatch.java` | parse(input) — 解析 `/<name>` 形态 |
| 新建 | `src/test/java/dev/cortex/command/DispatchTest.java` | parse 各种输入的单测 |
| 新建 | `src/main/java/com/cortex/command/Ui.java` | Ui 接口 + NopUi 测试桩 |
| 新建 | `src/main/java/com/cortex/command/BuiltinLocal.java` | 5 条纯本地命令(/help /status /memory /permission /session) |
| 新建 | `src/main/java/com/cortex/command/BuiltinUi.java` | 5 条影响界面命令(/exit /plan /compact /resume /clear) |
| 新建 | `src/main/java/com/cortex/command/BuiltinPrompt.java` | 2 条提示词命令(/do /review) + REVIEW_DIRECTIVE 常量 |
| 新建 | `src/main/java/com/cortex/command/Builtins.java` | registerAll(reg) 把 12 条命令一次性注入 |
| 新建 | `src/test/java/dev/cortex/command/BuiltinsTest.java` | 12 条命令注册成功、NopUi 调用全部 handler 不抛异常 |
| 改造 | `src/main/java/com/cortex/tui/Commands.java` | 删旧 builtinCommands + handle* 方法; 新增 CortexModel 实现 Ui 接口的全部方法 + dispatchSlash 入口 |
| 新建 | `src/main/java/com/cortex/tui/CompletionMenu.java` | CompletionMenu 类 + handleCompletionKey + syncCompletionFromInput + render |
| 改造 | `src/main/java/com/cortex/tui/Resume.java` | handleResume 改名/拆分为 CortexModel.openResumeMenu(Ui 接口实现) |
| 改造 | `src/main/java/com/cortex/tui/CortexModel.java` | 加 cmdRegistry+completion+pendingPrintln+pendingAction 字段; 构造 registry; IDLE 输入分支接入补全键位拦截 |
| 改造 | `src/main/java/com/cortex/tui/AgentEvent 队列.java` | submit() 把 dispatchCommand 调用替换为 dispatchSlash |
| 改造 | `src/main/java/com/cortex/tui/Styles.java + MarkdownRenderer.java` | render() 在 TextBox 渲染后、status bar 前插入 completion.render |
| 改造 | `src/test/java/dev/cortex/tui/CortexModelTest.java` | 老的 TestTuiSlashCompactRoutesToCommand / TestTuiUnknownSlashCommandFriendly 迁到新分发器 |
| 改造 | `src/main/java/com/cortex/prompt/Prompt.java` | READY_HINT 改为"建议输入 /help 查看命令"(不再列具体命令名) |
| 改造 | `src/test/java/dev/cortex/prompt/PromptTest.java` | 跟随 READY_HINT 变化 |
| 改造 | `src/main/java/com/cortex/memory/MemoryManager.java` + 单测 | 新增 listFiles() |
| 改造 | `src/main/java/com/cortex/session/Writer.java` + 单测 | 新增 path() |
| 改造 | `src/main/java/com/cortex/tool/ToolRegistry.java` | 新增 count() |
| 改造 | `src/main/java/com/cortex/agent/SessionRuntime.java` + 单测 | 新增 resetForNewSession |

## 任务

### T0a: MemoryManager.listFiles

**文件**：`src/main/java/com/cortex/memory/MemoryManager.java`、`src/test/java/dev/cortex/memory/MemoryManagerTest.java`
**依赖**：无
**步骤**：
...
**文件**：`src/main/java/com/cortex/session/Writer.java`、`src/test/java/dev/cortex/session/WriterTest.java`
**依赖**：无
**步骤**：
...
**文件**：`src/main/java/com/cortex/agent/SessionRuntime.java`、`src/test/java/dev/cortex/agent/SessionRuntimeTest.java`、`src/main/java/com/cortex/tool/ToolRegistry.java`
**依赖**：无
**步骤**：
...
**文件**：`src/main/java/com/cortex/command/Kind.java`、`src/main/java/com/cortex/command/Command.java`
**依赖**：无
**步骤**：
1. 新建包 `com.cortex.command`
...
**文件**：`src/main/java/com/cortex/command/CommandRegistry.java`、`src/test/java/dev/cortex/command/CommandRegistryTest.java`
**依赖**：T1
**步骤**：
...
**文件**：`src/main/java/com/cortex/command/Dispatch.java`、`src/test/java/dev/cortex/command/DispatchTest.java`
**依赖**：无
**步骤**：
...
**文件**：`src/main/java/com/cortex/command/Ui.java`
**依赖**：无（声明 Ui 接口让 T1 的 Handler 签名合法）
**步骤**：
1. import `com.cortex.permission.Mode`
...
**文件**：`src/main/java/com/cortex/command/BuiltinLocal.java`
**依赖**：T1、T2、T4
**步骤**：
1. `static Handler help(CommandRegistry reg)`：返回 lambda,handler 内调 `reg.visible()`,计算最长 `name` 长度做对齐填充,逐条拼 `"/<name>  <desc>"`,用 `\n` 连接后 `ui.println(...)` 一次输出
2. `static Handler status()`：6 行 key:value,key 列宽固定（`Mode:` `Tokens:` `Tools:` `Memories:` `Model:` `Directory:` 中最长那个）；值依次取 `ui.mode().toString()`、`String.format("%d in / %d out", ui.usageIn(), ui.usageOut())`、`String.format("%d enabled", ui.toolCount())`、`String.format("%d files", ui.memoryFiles().size())`、`ui.modelName()`、`ui.cwd()`；首行加标题 `"Cortex Status"`（空行隔开）
...
**文件**：`src/main/java/com/cortex/command/BuiltinUi.java`
**依赖**：T1、T4
**步骤**：
...
**文件**：`src/main/java/com/cortex/command/BuiltinPrompt.java`
**依赖**：T1、T4
**步骤**：
1. 包内 `static final String REVIEW_DIRECTIVE = "请审查当前上下文中的代码变更/已读取的文件,指出潜在 bug、可读性问题和可简化处。";`
2. `static Handler doRun()`：`(cancelled, ui) -> { ui.setMode(Mode.DEFAULT); ui.injectAndSend("/do", Prompt.EXECUTE_DIRECTIVE); }`（import `com.cortex.prompt.Prompt`）
...
**文件**：`src/main/java/com/cortex/command/Builtins.java`、`src/test/java/dev/cortex/command/BuiltinsTest.java`
**依赖**：T5、T6、T7
**步骤**：
...
### T8.5: CortexModel 字段铺垫

**文件**：`src/main/java/com/cortex/tui/CortexModel.java`
**依赖**：T8
**步骤**：
1. `CortexModel` 类在原 `toolRegistry: ToolRegistry` 字段之后增 `cmdRegistry: CommandRegistry`、`completion: CompletionMenu`、`pendingPrintln: List<String>`（`new ArrayList<>()`）、`pendingAction: Runnable` 四个字段
2. 在构造函数中初始化（registry 通过 T9c 注册）

**验证**：`./gradlew compileJava` 零退出码

### T9a: CortexModel 实现 Ui 只读查询方法

**文件**：`src/main/java/com/cortex/tui/Commands.java`（在该文件已被清空旧 handler 后重写）
**依赖**：T8.5
**步骤**：
...
2. 给 `CortexModel` 实现 `Ui` 接口的所有只读方法（写在 `CortexModel.java` 内 `implements Ui` 即可,Commands.java 可只保留 helper 函数）：
   - `mode() -> this.mode`
   - `usageIn() / usageOut() -> this.usageIn / this.usageOut`
   - `modelName() -> provider != null ? provider.model() : ""`
   - `cwd() -> this.cwd`（若 `CortexModel` 上没有此字段,从构造参数拷一份）
...
### T9b: CortexModel 实现 Ui 写入方法 + 缓冲机制

**文件**：`src/main/java/com/cortex/tui/Commands.java`、`src/main/java/com/cortex/tui/CortexModel.java`
**依赖**：T9a
**步骤**：
...
7. `clearAndNewSession()` — 步骤：
   ...
   b. `var newSesCtx = ContextCompactor.newSessionContext(this.cwd);` `IOException` 时 `error(ex.getMessage())` 直接返回（不动现状）。注意签名：`ContextCompactor.newSessionContext(Path workspace) throws IOException`,内部在 `<workspace>/.cortex/sessions/<id>/tool-results` 下建好目录
...
**依赖**：T9b
**步骤**：
1. `Commands.java` 新增静态方法或 `CortexModel.dispatchSlash(String text): boolean`：
...
**文件**：`src/main/java/com/cortex/tui/CompletionMenu.java`（新）
**依赖**：T2
**步骤**：
...
**依赖**：T9c、T10、T11
**步骤**：
1. `CortexModel` 字段已在 T8.5 加好（`cmdRegistry`、`completion`、`pendingPrintln`、`pendingAction`）；本任务不重新声明
...
**文件**：`src/main/java/com/cortex/tui/AgentEvent 队列.java`、`src/main/java/com/cortex/tui/Styles.java + MarkdownRenderer.java`
**依赖**：T12
**步骤**：
...
**验证**：`./gradlew shadowJar` 通过；`./gradlew test -Dtest=CortexModelTest#testTuiSlash` 期待红（下一任务迁移测试）

### T14: 迁移测试 + READY_HINT 调整

**文件**：`src/test/java/dev/cortex/tui/CortexModelTest.java`、`src/main/java/com/cortex/prompt/Prompt.java`、`src/test/java/dev/cortex/prompt/PromptTest.java`
**依赖**：T13
**步骤**：
1. `CortexModelTest`：把 `testTuiSlashCompactRoutesToCommand` 改为构造 `CortexModel` + 注册 builtins 后,调 `app.dispatchSlash("/compact")`、断言返回 `true`、断言未调 `conv.addUser`
2. `CortexModelTest`：`testTuiUnknownSlashCommandFriendly` 改为调 `app.dispatchSlash("/foobar")`、断言返回 `true`、断言 `app.pendingPrintln()` 含"未知命令"
3. `CortexModelTest`：新增 `testTuiDispatchCaseInsensitive` 测 `/Help` 与 `/help` 同效；`testTuiDispatchHelpListsAllBuiltins` 测 /help 输出含 12 个命令名
...
**依赖**：T14
**步骤**：
1. `cd /Users/codemelo/cortex && ./gradlew shadowJar`
2. `tmux new-session -d -s mewspec 'java -jar build/libs/cortex.jar'` 启动会话
...
- T8 必须在 T5+T6+T7 后
- T8.5 给 `CortexModel` 加字段,作为 T9a 前置
