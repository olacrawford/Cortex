...
- [ ] `com.mewcode.subagent` 包存在且编译通过(验证:`./gradlew compileJava`)
...
- [ ] `MewCodeModel` 持有 `taskMgr` 与 `subAgentCatalog` 字段(验证:`./gradlew compileJava`)
- [ ] `init()` 启动 `consumeTaskDone` virtual thread,任务完成时把 `<task-notification>` 注入 `runtime.pendingReminders`(验证:`./gradlew test -Dtest=MewCodeModelTest#consumeTaskDone`)
- [ ] `tui/SkillFork.java` 改造为调 `Agent.runToCompletion` 而非自己拼装循环(验证:现有 skills 测试不破)
- [ ] `MewCode.java` 注册 4 个 task 工具 + 1 个 Agent 工具,`subAgentCatalog` 与 `taskMgr` 传给 `MewCodeModel`(验证:`./gradlew shadowJar`)
...
**步骤:**
- [ ] tmux 启动 mewcode:`tmux new-session -d -s ch13 -x 200 -y 50 "java -jar build/libs/mewcode.jar"`
- [ ] 给 LLM 输入:「用 Explore 子 Agent 找出 `src/main/java/com/mewcode/permission` 包下所有以 `test` 开头的方法,只统计数量,不要修改任何文件」
...
**步骤:**
- [ ] tmux 启动 mewcode
- [ ] 第一轮:让 LLM 读一些文件铺垫上下文,如「读 `src/main/java/com/mewcode/agent/Agent.java` 头 50 行」
...
**步骤:**
- [ ] tmux 启动 mewcode
- [ ] 输入:「用 `run_in_background=true name=worker1` 起一个 general-purpose 子 Agent,任务是『列出 `src/main/java/com/mewcode/MewCode.java` 的 import 块』」
- [ ] 主 Agent 收到 task_id,等几秒后 task-notification 显示 Result(imports 列表)
- [ ] 输入:「调 SendMessage 给 worker1 发『再列出 `src/main/java/com/mewcode/agent/Agent.java` 头 20 行』」
...
**步骤:**
- [ ] tmux 启动 mewcode
- [ ] 输入:「用 Agent 工具调 `subagent_type=wc-counter`,任务: 统计 `README.md` 和 `src/main/java/com/mewcode/MewCode.java` 的行数」
