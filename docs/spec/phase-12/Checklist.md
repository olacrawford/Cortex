...
- [x] `com.cortex.subagent` 包存在且编译通过(验证:`./gradlew compileJava`)
...
- [x] `CortexModel` 持有 `taskMgr` 与 `subAgentCatalog` 字段(验证:`./gradlew compileJava`)
- [x] `init()` 启动 `consumeTaskDone` virtual thread,任务完成时把 `<task-notification>` 注入 `runtime.pendingReminders`(验证:`./gradlew test -Dtest=CortexModelTest#consumeTaskDone`)
- [x] `tui/SkillFork.java` 改造为调 `Agent.runToCompletion` 而非自己拼装循环(验证:现有 skills 测试不破)
- [x] `Cortex.java` 注册 4 个 task 工具 + 1 个 Agent 工具,`subAgentCatalog` 与 `taskMgr` 传给 `CortexModel`(验证:`./gradlew shadowJar`)
...
**预置:** 无须额外配置。当前目录 `cd /Users/codemelo/cortex`。

**步骤:**
- [x] tmux 启动 cortex:`tmux new-session -d -s ch13 -x 200 -y 50 "java -jar build/libs/cortex.jar"`
- [x] 给 LLM 输入:「用 Explore 子 Agent 找出 `src/main/java/com/cortex/permission` 包下所有以 `test` 开头的方法,只统计数量,不要修改任何文件」
...
**步骤:**
- [x] tmux 启动 cortex
- [x] 第一轮:让 LLM 读一些文件铺垫上下文,如「读 `src/main/java/com/cortex/agent/Agent.java` 头 50 行」
...
**预置:** 接场景 2 之后,或者重启 cortex 后先 launch 一个长跑任务。
...
**步骤:**
- [x] tmux 启动 cortex(权限模式 default,未预先批准 echo)
...
**步骤:**
- [x] tmux 启动 cortex
- [x] 输入:「用 `run_in_background=true name=worker1` 起一个 general-purpose 子 Agent,任务是『列出 `src/main/java/com/cortex/Cortex.java` 的 import 块』」
- [x] 主 Agent 收到 task_id,等几秒后 task-notification 显示 Result(imports 列表)
- [x] 输入:「调 SendMessage 给 worker1 发『再列出 `src/main/java/com/cortex/agent/Agent.java` 头 20 行』」
...
**预置:** 创建 `.cortex/agents/wc-counter.md`:
```yaml
---
name: wc-counter
...
**步骤:**
- [x] tmux 启动 cortex
- [x] 输入:「用 Agent 工具调 `subagent_type=wc-counter`,任务: 统计 `README.md` 和 `src/main/java/com/cortex/Cortex.java` 的行数」
...
**预置:** 创建 `.cortex/agents/bad.md` 含非法字段:
```yaml
---
name: bad
description: 字段错误测试
...
- [x] **测试完删除 `.cortex/agents/bad.md`**

### 场景 11:角色文件覆盖

**预置:** 创建 `.cortex/agents/explore.md`:
```
---
name: Explore
...
- [x] 删除 `.cortex/agents/explore.md`,重启 cortex,再次跑 → 不再含此标记