...
- [ ] `TeamManager.create("demo", "")` 在 `~/.mewcode/teams/demo/config.json` 落地(验证:运行单测后检查文件存在)
- [ ] `TeamManager.create("foo bar/baz", "")` sanitize 后路径为 `~/.mewcode/teams/foo-bar-baz/`(验证:单测)
...
- [ ] `spawnTeammate` 创建 worktree 路径为 `.mewcode/worktrees/team-demo+alice`(验证:单测/集成测试)
...
- [ ] Pane 后端的 `mewcode --team-member` 子进程**不启动 TUI**,跑 `TeamMemberRunner.run` 自治循环——读 mailbox → `runToCompletion` → 通知 Lead idle → stdin wake 等下一轮(验证:tmux 实跑看 alice pane 显示纯文本日志流而非 mewcode TUI 框)
...
- [ ] `cd /path/to/mewcode && ./gradlew shadowJar`(预编译,加快冷启动)
- [ ] `java -jar build/libs/mewcode.jar` 启动 TUI;启动消息显示一切正常,无 ch15 相关 error
- [ ] 在 TUI 输入:「创建一个名为 demo 的团队」
  - 预期:Agent 调 `TeamCreate(teamName="demo")`;返回 `{"teamName":"demo","backend":"tmux","configPath":"..."}`
  - 验证:`ls ~/.mewcode/teams/demo/config.json` 存在;`cat config.json` 中 `backend` 字段为 `tmux`
- [ ] 在 TUI 输入:「派 alice 用 general-purpose 角色,在 worktree 里跑 `echo hello > /tmp/test_alice.txt && pwd > /tmp/test_alice_pwd.txt`」
  ...
  - 验证 f:`cat ~/.mewcode/teams/demo/config.json` 中 `members` 数组含 alice,`backendType="tmux"`,`paneId` 非空
  - 验证 g:`~/.mewcode/teams/demo/mailbox/<aliceAgentId>.json` 中应已含一条 from=lead 的 text 消息——Pane 后端的 initialPrompt 预写入证据
...
- [ ] 等待 alice 任务自然结束(或在 TUI 输入 `/team kill alice` 终止)
  - 验证 a:`cat ~/.mewcode/teams/demo/config.json` 中 alice 的 `isActive` 为 `false`(跨进程 reload 修复——alice 子进程的 `setMemberActive(false)` 必须真的反映到 disk;早期 bug 是静默 no-op)
  - 验证 b:Lead 的 mailbox(`cat ~/.mewcode/teams/demo/mailbox/lead.json`)含一条 `summary` 含 `idle` 的消息,且 1-2 秒后该消息 `read=true`(watcher 已消费)
  - 验证 c:Lead 屏幕**不需要用户输入**自动出现 `● [team-update] 队员发来新消息...` 文本块 + 紧接的 Synthesis 回复(自动唤醒)
- [ ] 在 TUI 输入 `/team delete demo --force`
  - 验证 a:`ls ~/.mewcode/teams/` 无 `demo` 目录
...
步骤:
- [ ] 启动 `java -jar build/libs/mewcode.jar`(同会话已 unset 上述变量)
- [ ] 在 TUI 输入:「创建 inproc 团队」
  - 验证:`cat ~/.mewcode/teams/inproc/config.json` 中 `backend` 为 `in-process`
...
步骤:
- [ ] `MEWCODE_COORDINATOR_MODE=1 java -jar build/libs/mewcode.jar`
...
- [ ] 在 TUI 输入:「跑 `git status`」
  ...
  - 验证:输出含 git 状态信息
- [ ] 在 TUI 输入:「派几个队员探索 mewcode 的 dev/mewcode/agent 和 dev/mewcode/team」
...
步骤:
- [ ] 准备一个角色定义 `~/.mewcode/agents/planner.md`,frontmatter 含 `permissionMode: plan`,body 简述「先制定计划」
- [ ] 启动 mewcode,创建 team `plan-test`
...
- [ ] mewcode 启动时 `~/.mewcode/teams/` 不存在,自动创建,不报错
- [ ] `~/.mewcode/teams/<somename>/config.json` 内容损坏时,启动只 stderr 警告,跳过该 Team
