...
- [ ] 不破坏 ch04/ch05：多轮连环、用户取消、流出错恢复、历史一致、缓存命中、规划按轮次注入仍成立（验证：跑既有端到端关键场景；`./gradlew test` 通过）。(AC16/N3)

## 编译与测试
- [ ] `./gradlew shadowJar` 无错误（fat jar 可启动）。
- [ ] `./gradlew test` 通过（config、conversation、tool、agent、prompt、permission、tui 单测）。
- [ ] 取消/阻塞类用例在 JUnit `@Timeout(5s)` 与 surefire `<forkedProcessTimeoutInSeconds>` 内退出，无 virtual thread 泄漏（重点守护人在回路阻塞/取消）。(N4)
- [ ] `./gradlew compileJava` (代码风格由 IDE 保证) 无 diff（google-java-format 合规）；如启用，`./gradlew spotbugs:check` 无致命告警。(AC17/N8)
