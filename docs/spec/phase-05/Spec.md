# 权限系统 Spec## 背景ch05 做完系统提示工程化后，Cortex 已能自主多轮想 → 调工具 → 看结果地干活。但**工具执行是完全裸奔的**：工具执行链路上没有任何闸门——写文件工具能写任意路径、改文件工具能改任意文件、命令执行能跑任意命令（含 `rm -rf /`）、读写类工具能触碰项目目录之外的任何位置。这在 ch02 背景里就被列为预留能力（「在没有工具调用、**权限**、记忆等高级能力之前」）。

ch06 给 Cortex 装上**五层防御的权限系统**：危险命令黑名单硬拦截、路径沙箱限定操作范围、可配置规则做细粒度放行/拦截、多档权限模式覆盖信任等级、人在回路作为最后确认。五层串联、逐层短路，一次工具调用在真正执行前必须穿过这道护栏。权限被拒绝时把结构化拒绝**回灌**给模型让它调整策略，而非终止 Agent Loop。这是从「会自己干」到「在安全护栏内自己干」的关键一跃。
