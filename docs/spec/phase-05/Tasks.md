...
| 操作 | 文件 | 职责 |
|------|------|------|
| 新建 | `src/main/java/com/cortex/permission/Mode.java` | `Mode` 四档 enum + `displayName`/`parse` |
| 新建 | `src/main/java/com/cortex/permission/Decision.java` | `enum Decision { ALLOW, DENY, ASK }` |
| 新建 | `src/main/java/com/cortex/permission/Category.java` | `enum Category { READ, WRITE, EXEC }` |
| 新建 | `src/main/java/com/cortex/permission/Outcome.java` | `enum Outcome { DENY_ONCE, ALLOW_ONCE, ALLOW_FOREVER }` |
| 新建 | `src/main/java/com/cortex/permission/Blacklist.java` | 内置危险命令正则集 + `hitsBlacklist`（不可配，N1） |
| 新建 | `src/main/java/com/cortex/permission/Sandbox.java` | `resolveRoot`、`sandboxOK`、`evalSymlinksOrAncestor`（N2） |
| 新建 | `src/main/java/com/cortex/permission/Rule.java` | `record Rule`、`parseRule`、`matchPattern`（glob） |
| 新建 | `src/main/java/com/cortex/permission/RuleSet.java` | 单层规则集 + `match()`（deny 先于 allow） |
| 新建 | `src/main/java/com/cortex/permission/Settings.java` | `record Settings`、`loadSettings`（SnakeYAML Engine）、`toRuleSet`、`FriendlyNames`、`Categorizer`、`TargetExtractor` |
| 新建 | `src/main/java/com/cortex/permission/PermissionEngine.java` | `PermissionEngine`、`create`、`check` 前四层流水线、`modeFallback`、`startMode`、`CheckResult` record |
| 新建 | `src/main/java/com/cortex/permission/Persister.java` | `ruleFor`、`persistLocalAllow`（写本地层文件） |
| 新建 | `src/test/java/dev/cortex/permission/*Test.java` | 黑名单/沙箱(含祖先回退)/规则/优先级/矩阵/加载降级/解析失败 单测（JUnit 5） |
| 改   | `src/main/java/com/cortex/agent/Agent.java` | 删 `Mode`（迁 permission）；`Agent` 加 `engine`；`executeBatched(+mode)` 接入 `check`；`requestApproval`；`ApprovalRequest`/`AgentEvent.Approval`；Deny 用 `ToolResult` 构造 |
| 新建 | `src/main/java/com/cortex/agent/ApprovalRequest.java` | `record ApprovalRequest(String name, String args, String reason, BlockingQueue<Outcome> respond)`（可作为 `Agent` 嵌套 record） |
| 改   | `src/test/java/dev/cortex/agent/AgentTest.java` | 权限集成(Allow/Deny/Ask/会话/永久)、保序回灌、只读并发不退化、取消、模式迁移 |
| 改   | `src/main/java/com/cortex/tui/CortexModel.java` | `mode`→`permission.Mode`、加 `engine`/`pending`/`approveCursor`；构造增参；`APPROVING` 分派；全局 Ctrl+C/Esc 覆盖 approving；`ReverseTab`（Shift+Tab）循环模式(`nextMode`) |
| 改   | `src/main/java/com/cortex/tui/AgentEvent 队列.java` | `handleEvent` 处理 `Approval`；`updateApproving`；`sendOutcome`；`submit` 保留 `/plan`·`/do`（去掉 `/mode`）；`beginTurn` 传 engine |
| 改   | `src/main/java/com/cortex/tui/Styles.java + MarkdownRenderer.java` | `statusBar` 左侧常驻模式（取代 provider 名）；待批准块渲染（多行三选菜单 + 光标高亮） |
| 改   | `src/test/java/dev/cortex/tui/CortexModelTest.java` | `ReverseTab` 循环切换、approval 态按键回传、Esc 取消兜底、状态栏显示模式、模式跨轮保持；既有 `/plan`·`/do` 用例适配新 `Mode` 枚举 |
| 改   | `src/main/java/com/cortex/Cortex.java` | 构造 `PermissionEngine.create(root)` 注入 `CortexModel` |
| 改   | `src/main/java/com/cortex/smoke/Cortex.java` | 新增 `cwd`、构造引擎、`Mode.BYPASS` 运行；`new Agent(...)` 增参 |
| 改   | `build.gradle.kts` | 加 `com.fasterxml.jackson.core:jackson-databind` 依赖（解析 `ToolCall.input` JSON） |
| 改   | `.gitignore` | 追加 `.cortex/settings.local.yaml` |
| 新建 | `.cortex/settings.yaml.example` | 权限配置示例（defaultMode + allow/deny） |

---

## T1: permission 基础类型
**文件：** `src/main/java/com/cortex/permission/{Mode,Decision,Category,Outcome}.java`
**依赖：** 无
**步骤：**
...
## T2: 危险命令黑名单
**文件：** `src/main/java/com/cortex/permission/Blacklist.java`
**依赖：** 无
**步骤：**
...
## T3: 路径沙箱
**文件：** `src/main/java/com/cortex/permission/Sandbox.java`
**依赖：** 无
**步骤：**
...
## T4: 规则与匹配
**文件：** `src/main/java/com/cortex/permission/{Rule,RuleSet}.java`
**依赖：** 无
**步骤：**
...
## T5: 配置加载与映射
**文件：** `src/main/java/com/cortex/permission/Settings.java`
**依赖：** T1, T4
**步骤：**
...
## T6: 引擎与前四层流水线
**文件：** `src/main/java/com/cortex/permission/PermissionEngine.java`
**依赖：** T1, T2, T3, T4, T5
**步骤：**
...
2. `static PermissionEngine create(Path root)`：
   ...
   - 加载三层：user=`<user.home>/.cortex/settings.yaml`、project=`<root>/.cortex/settings.yaml`、local=`<root>/.cortex/settings.local.yaml`；各 `loadSettings`→`toRuleSet`；**单个文件读/解析失败仅降级跳过该文件（视为空），绝不抛致命异常**。
   - `localPath = root.resolve(".cortex/settings.local.yaml")`。
...
## T7: 永久规则写入
**文件：** `src/main/java/com/cortex/permission/Persister.java`
**依赖：** T5, T6
**步骤：**
...
**文件：** `src/main/java/com/cortex/agent/Agent.java`、`src/main/java/com/cortex/agent/ApprovalRequest.java`
**依赖：** T6, T7
**步骤：**
1. **模式迁移**：删除 agent 内 `Mode`/`NORMAL`/`PLAN` 定义，import `com.cortex.permission.Mode`，全部改用 `Mode.DEFAULT`/`Mode.PLAN`；`run` 形参 `Mode mode`；`mode == Mode.PLAN` 处不变（defs 选只读、`PlanReminder` 注入）。
...
**验证：** `./gradlew test -Dtest='com.cortex.tui.*'` 通过。

## T12: Main / smoke / 配置文件接线
**文件：** `src/main/java/com/cortex/Cortex.java`、`src/main/java/com/cortex/smoke/Cortex.java`、`build.gradle.kts`、`.gitignore`、`.cortex/settings.yaml.example`
**依赖：** T6, T8, T10
**步骤：**
1. `Cortex.java`：`Path root = Path.of("").toAbsolutePath();`（必要时 `try { root = root.toRealPath(); } catch(IOException ignore) {}`）；`PermissionEngine engine = PermissionEngine.create(root);`（create 内部已把 IO 失败降级 + stderr 警告）；`new CortexModel(cfg.providers(), version, registry, engine).run();`。
2. `smoke/Cortex.java`：新增 `Path cwd = Path.of("").toAbsolutePath();`；`PermissionEngine engine = PermissionEngine.create(cwd);`；`new Agent(provider, DefaultToolRegistry.create(), "dev", engine)`；`run(conv, Mode.BYPASS)`。
...
4. `.gitignore`：在「本地配置」段追加 `.cortex/settings.local.yaml`。
5. `.cortex/settings.yaml.example`：示例——`defaultMode: default`；`permissions.allow: ["Bash(git *)", "Bash(./gradlew test)"]`；`permissions.deny: ["Bash(rm *)", "Read(.env)", "Write(.env)"]`；注释说明三层文件与优先级，并注明**只读类默认即 Allow，allow 规则主要用于提前放行 Bash/Write，deny 规则可对只读做围栏（如 Read(.env)）**。

**验证：** `./gradlew shadowJar` 全绿；`./gradlew exec:java -Dexec.mainClass=com.cortex.smoke.Main` 在含 `WriteFile` 的脚本下**不阻塞、跑完**（确认 `Mode.BYPASS` 跳过 Ask）；`java -jar build/libs/cortex.jar` 能正常启动进对话。
...
4. 确认 `.cortex/settings.local.yaml` 已被 gitignore（`git check-ignore`）；检索输出无 `api_key` 明文。
