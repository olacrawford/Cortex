...
- [ ] permission 包零改动：`git diff src/main/java/com/mewcode/permission/` 在本次开发期间无任何修改(验证：本章结束时核对 diff 范围)。(N4)
- [ ] provider 适配层零改动：`src/main/java/com/mewcode/llm/AnthropicProvider.java`、`src/main/java/com/mewcode/llm/OpenAIProvider.java` 无修改(验证：核对 diff)。(AC12/N3)
...
- [ ] `./gradlew test -Dtest='com.mewcode.mcp.*' -Dsurefire.rerunFailingTestsCount=3` 反复跑 3 轮无偶发失败(重点守护 `McpManager` 并发连接、共享状态、`close` 兜底)。(N7/N8)
