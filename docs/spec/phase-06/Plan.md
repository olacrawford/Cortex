...
> 技术栈：Java 21（LTS；virtual thread + `BlockingQueue`）+ Gradle；使用 **官方 Java SDK** `io.modelcontextprotocol.sdk:mcp` 承载协议层（JSON-RPC 编解码、initialize 握手、stdio 与 Streamable HTTP 传输）。本章新增 **`com.mewcode.mcp` 包** 与 `MewCode` 装配,**不改 tool / agent / tui / permission / llm / config / conversation / prompt**。

## 架构概览

- **`com.mewcode.mcp` 包（新增）**：承载 MCP 客户端的全部职责——配置加载与两层合并、`${VAR}` 展开、字段校验、调用 SDK 建立 stdio / HTTP 会话、把远端工具适配成内置 `Tool`、统一管理生命周期。仅依赖 `com.mewcode.tool`、SDK 与 JDK 标准库；不依赖 agent / tui / permission / conversation。
- **`MewCode`（改造）**：在 `ToolRegistry.defaults()` 之后、`PermissionEngine` 与 `MewCodeModel` 之前,加载 mcp 配置 → 启动 `McpManager` → 把 Manager 产出的工具注册进 registry → 退出时通过 `Runtime.getRuntime().addShutdownHook(...)`（或 try-with-resources）触发 `manager.close()`。
...
### 工具适配（包内私有）
```java
// McpTool 实现 com.mewcode.tool.Tool。
final class McpTool implements com.mewcode.tool.Tool {
...
public java.util.List<com.mewcode.tool.Tool> tools();
...
**职责：** 把 SDK 返回的 `McpSchema.Tool` 适配为 `com.mewcode.tool.Tool`。
**关键点：**
...
### `dev/mewcode/SmokeMewCode.java`(不改)
...
├── src/main/java/com/mewcode/MewCode.java — 改:装配 McpManager,注册 MCP 工具,addShutdownHook(mgr::close)
...
MewCode.main()
  ...
  ├─ new MewCodeModel(.registry(registry).engine(engine).build().run();
