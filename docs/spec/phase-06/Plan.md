...
> 技术栈：Java 21（LTS；virtual thread + `BlockingQueue`）+ Gradle；使用 **官方 Java SDK** `io.modelcontextprotocol.sdk:mcp` 承载协议层（JSON-RPC 编解码、initialize 握手、stdio 与 Streamable HTTP 传输）。本章新增 **`com.cortex.mcp` 包** 与 `Cortex` 装配,**不改 tool / agent / tui / permission / llm / config / conversation / prompt**。

## 架构概览

- **`com.cortex.mcp` 包（新增）**：承载 MCP 客户端的全部职责——配置加载与两层合并、`${VAR}` 展开、字段校验、调用 SDK 建立 stdio / HTTP 会话、把远端工具适配成内置 `Tool`、统一管理生命周期。仅依赖 `com.cortex.tool`、SDK 与 JDK 标准库；不依赖 agent / tui / permission / conversation。
- **`Cortex`（改造）**：在 `ToolRegistry.defaults()` 之后、`PermissionEngine` 与 `CortexModel` 之前,加载 mcp 配置 → 启动 `McpManager` → 把 Manager 产出的工具注册进 registry → 退出时通过 `Runtime.getRuntime().addShutdownHook(...)`（或 try-with-resources）触发 `manager.close()`。
...
### 工具适配（包内私有）
```java
// McpTool 实现 com.cortex.tool.Tool。
final class McpTool implements com.cortex.tool.Tool {
...
// - root: 项目根(用来定位 <root>/.cortex/mcp.yaml)
...
// version 透传到 implementation.version(便于 server 端识别 cortex 版本)。
public static McpManager start(McpConfig cfg, String version);

// 返回适配好的工具列表(按 server 名 → 工具名 稳定排序)。
public java.util.List<com.cortex.tool.Tool> tools();
...
### `dev/cortex/mcp/ConfigLoader.java`
...
- `loadConfig(Path root)`:
  - 用户级 = `System.getProperty("user.home")` 取家目录 + `/.cortex/config.yaml`;项目级 = `root.resolve(".cortex/mcp.yaml")`。
  - 两层各自 `loadFile` + `applyExpansion`;任一层解析失败 stderr 一行告警并跳过(该层视为空)。
  - `mergeServers` 后逐个 `validateServer`,组装 `McpConfig`。

### `dev/cortex/mcp/McpManager.java`
...
- `start(cfg, version)`:
  ...
  - `connectOne(...)` 内:
    ...
    - `McpAsyncClient client = McpClient.async(transport).clientInfo(new Implementation("cortex", version)).build();`
...
### `dev/cortex/mcp/McpTool.java`
**职责：** 把 SDK 返回的 `McpSchema.Tool` 适配为 `com.cortex.tool.Tool`。
**关键点：**
...
### `dev/cortex/SmokeCortex.java`(不改)
...
├── src/main/java/com/cortex/Cortex.java — 改:装配 McpManager,注册 MCP 工具,addShutdownHook(mgr::close)
...
| 配置文件位置 | 项目级 `<root>/.cortex/mcp.yaml` + 用户级 `~/.cortex/mcp.yaml` | 用户拍板;项目级 dotfile 一眼可见、与现有 `.cortex/config.yaml`(providers 凭据)分离 |
...
| 未定义变量 | 空串 + 一次性告警(不阻断) | server 自决无凭据时是否能跑;cortex 不替它拍板 |
...
| 异步桥接 | SDK 的 `Mono<T>` 一律 `.block(Duration.ofSeconds(30))` 转同步;调用层用 `CompletableFuture.orTimeout` 二次兜底 | cortex 内部以同步阻塞为主线(virtual thread 顶替线程池);避免把 Reactor 类型外泄到 mcp 包之外 |
...
Cortex.main()
  ...
  ├─ new CortexModel(.registry(registry).engine(engine).build().run();
