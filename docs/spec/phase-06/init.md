# 我的初步想法
这一步的目标是：实现一个 MCP 客户端，让 Cortex 在启动时自动发现并注册外部 MCP Server 提供的工具。用户在配置文件里声明 Server 列表，Cortex 就能通过标准化的 MCP 协议把这些工具无缝接进工具中心，Agent 使用时完全无感。
...
- 用适配层把发现到的远端工具包装成 Cortex 已有的 Tool 接口注册进去，Agent 调用时无感
