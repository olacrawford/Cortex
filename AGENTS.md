...
## What this is
Cortex — a Claude Code-style terminal AI coding assistant, written in Java. Phase-01 (multi-protocol LLM terminal chat client), phase-02 (tool system), phase-03 (Agent Loop: multi-round ReAct, batched concurrent execution, Plan Mode), phase-04 (system prompt engineering: modular assembly, env info, cache channels, system-reminder), phase-05 (permission system: 5-layer defense, human-in-the-loop approval, 3-tier YAML rules) and phase-06 (MCP client: config-driven server discovery, stdio + Streamable HTTP transports, tool adaptation with mcp__server__tool namespaces) are implemented on `feature/phase06`. Next spec phases go under `docs/spec/`.
