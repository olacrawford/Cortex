...
## What this is
Cortex — a Claude Code-style terminal AI coding assistant, written in Java. Currently mid-implementation of `docs/spec/phase-01` (multi-protocol LLM terminal chat client). Core layers (config, prompt, llm, conversation) + unit tests exist; the entrypoint and TUI are **not built yet**.

## Commands
- Build fat jar: `./gradlew shadowJar` → `build/libs/cortex.jar` (main class `com.cortex.Cortex`)
- Run: `java -jar build/libs/cortex.jar`
...
- **Config**: real keys live in `.cortex/config.yaml` (gitignored). Copy `.cortex/config.yaml.example` and fill in. Do not print `api_key` anywhere (spec N5).
...
- **Incomplete entrypoint**: `src/main/java/com/cortex/Cortex.java` is still the T1 placeholder (prints version); it does not load config or start the TUI. The `com.cortex.tui` / `tui.tea` packages referenced in `Plan.md`/`Tasks.md` do not exist yet. Next steps are Tasks T9–T11.
